import { ChatInputCommandInteraction, GuildEmoji, SlashCommandAttachmentOption, SlashCommandBuilder, SlashCommandIntegerOption, SlashCommandRoleOption } from "discord.js";
...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction){
        ...
        if(await RegisterRoleID(interaction.guildId, role_id, interaction.options.getInteger("votes", true))){
            ...
            message.awaitReactions({max: 1, time: 60_000, errors: ['time']}).then(collected =>{
                console.log("received reaction");
                const reaction = collected.first();
                console.log(interaction.guildId === null);
                console.log(reaction?.emoji);
                if(interaction.guildId === null || !reaction || (!reaction.emoji.id && !reaction.emoji.name)) return;
